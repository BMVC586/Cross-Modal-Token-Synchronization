...
class TransformerLightningModule(pl.LightningModule):
    def __init__(self, config: DictConfig):
        ...
        self.loss_audio_weight = config.optim.loss_audio_weight
        ...
        if config.model.bert.type == "huggingface":
            ...
            self.bert_attn_layers = BertModel(BertConfig(**config.model.bert))
        elif config.model.bert.type == "x-transformers":
            print("using x-transformers bert implementation")
            self.bert_attn_layers = Encoder(
    ...
    def forward(
        ...
        inputs_embeds = torch.cat((inputs_embeds, word_mask.unsqueeze(-1)), dim=-1)
        audio_tokens = audio_tokens[:, : inputs_embeds.size(1) * self.audio_alignment]
        seq_len = inputs_embeds.shape[1]

        cls_tokens = self.cls_token.expand(videos.size(0), -1, -1)
        inputs_embeds = self.emb_dropout_bert(
            torch.cat((cls_tokens, inputs_embeds), dim=1)
        )
        ...
        if self.is_train:
            ...
            if self.config.model.bert.type == "huggingface":
                output = self.bert_attn_layers(
                    inputs_embeds=inputs_embeds,
                    output_hidden_states=True,
                )
            elif self.config.model.bert.type == "x-transformers":
                output = self.bert_attn_layers(inputs_embeds)

            logits_category = self.category_classifier(output[:, 0, :])
            logits_audio = self.audio_classifier(output[:, 1:, :])
            logits_audio = logits_audio.unflatten(2, (-1, self.audio_vocab_size))

            # Horizontally Flipped visual predictions
            flipped_input_imbeds = self.forward_videos(T.functional.hflip(videos))
            flipped_input_imbeds = torch.cat(
                (flipped_input_imbeds, word_mask.unsqueeze(-1)), dim=-1
            )
            flipped_input_imbeds = self.emb_dropout_bert(
                torch.cat((cls_tokens, flipped_input_imbeds), dim=1)
            )
            if self.config.model.bert.type == "huggingface":
                flipped_output = self.bert_attn_layers(
                    inputs_embeds=flipped_input_imbeds,
                    output_hidden_states=True,
                )
            elif self.config.model.bert.type == "x-transformers":
                flipped_output = self.bert_attn_layers(flipped_input_imbeds)

            flipped_logits_category = self.category_classifier(flipped_output[:, 0, :])
            flipped_logits_audio = self.audio_classifier(flipped_output[:, 1:, :])
            flipped_logits_audio = flipped_logits_audio.unflatten(
                2, (-1, self.audio_vocab_size)
            )
            ...
            flipped_loss_category = F.cross_entropy(
                flipped_logits_category, labels, label_smoothing=self.label_smoothing
            )
            loss_category = (loss_category + flipped_loss_category) / 2

            # Audio classification Loss
            if self.config.train.method == "sync":
                # MLP Head for audio generation
                loss_audio = F.cross_entropy(
                    logits_audio.flatten(0, 2),
                    audio_tokens.flatten(),
                )
                flipped_loss_audio = F.cross_entropy(
                    flipped_logits_audio.flatten(0, 2),
                    audio_tokens.flatten(),
                )

                loss_audio = (loss_audio + flipped_loss_audio) / 2

            elif self.config.train.method == "autoregressive":
                ...
                flipped_loss_audio1 = self.audio_classifier_group1(
                    group1_audio_tokens,
                    context=flipped_output.hidden_states[-1][:, 1:, :],
                )
                flipped_loss_audio2 = self.audio_classifier_group2(
                    group2_audio_tokens,
                    context=flipped_output.hidden_states[-1][:, 1:, :],
                )
            ...
            # KL-Divergence Loss
            ## Audio
            loss_kl_audio = F.kl_div(
                F.log_softmax(logits_audio, dim=-1),
                F.softmax(flipped_logits_audio, dim=-1),
                reduction="batchmean",
            )
            loss_kl_audio += F.kl_div(
                F.log_softmax(flipped_logits_audio, dim=-1),
                F.softmax(logits_audio, dim=-1),
                reduction="batchmean",
            )
            loss_kl_audio /= 2 * seq_len

            ## Category
            loss_kl_category = F.kl_div(
                F.log_softmax(logits_category, dim=-1),
                F.softmax(flipped_logits_category, dim=-1),
                reduction="batchmean",
            )
            loss_kl_category += F.kl_div(
                F.log_softmax(flipped_logits_category, dim=-1),
                F.softmax(logits_category, dim=-1),
                reduction="batchmean",
            )
            loss_kl_category /= 2

            loss_kl_divergence = self.kl_alpha * (loss_kl_audio + loss_kl_category)

            loss_total = (
                loss_category + loss_audio * self.loss_audio_weight + loss_kl_divergence
            )

            if self.config.train.use_cutmix:
                label_args = labels.argmax(dim=-1)
                corrects = (logits_category + flipped_logits_category).topk(5, dim=1)[
                    1
                ] == label_args.unsqueeze(1)
            else:
                corrects = (logits_category + flipped_logits_category).topk(5, dim=1)[
                    1
                ] == labels.unsqueeze(1)
...
class DCTCNLightningModule(pl.LightningModule):
    def __init__(self, config: DictConfig):
        ...
        self.audio_classifier = nn.Linear(
            1664,
            self.audio_alignment * metadata.model.vq_groups * metadata.model.vq_vars,
        )
    ...
    def forward(
        ...
        loss_total = loss_category + loss_audio * self.loss_audio_weight
